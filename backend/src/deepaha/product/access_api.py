"""Same-origin access administration; all management writes require CSRF and operator."""
...
class AccountUpdate(Reason):
    active:bool
    roles:list[Literal['user','reviewer','operator']]=Field(min_length=1,max_length=3)
...
def mount_access(app,p,user):
    ...
    def invitations(req:Request,offset:int=Query(0,ge=0),limit:int=Query(30,ge=1,le=100)):
        return p.list_invitations(actor=user(req,'operator'),offset=offset,limit=limit)
    @app.post('/api/admin/invitations')
    def create(data:InvitationInput,req:Request):return p.create_invitations(actor=user(req,'operator',True),**data.model_dump())
    @app.post('/api/admin/invitations/{id}/revoke')
    def revoke(id:str,req:Request):return p.revoke_invitation(id,actor=user(req,'operator',True))
    @app.get('/api/admin/users')
    def users(req:Request,q:str=Query('',max_length=80),offset:int=Query(0,ge=0),limit:int=Query(30,ge=1,le=100)):
        return p.list_accounts(actor=user(req,'operator'),q=q,offset=offset,limit=limit)
    @app.patch('/api/admin/users/{id}')
    def update(id:str,data:AccountUpdate,req:Request):return p.update_account(id,actor=user(req,'operator',True),**data.model_dump())
    @app.post('/api/admin/users/{id}/revoke-sessions')
    def sessions(id:str,data:Reason,req:Request):return p.revoke_user_sessions(id,actor=user(req,'operator',True),reason=data.reason)
    @app.post('/api/admin/users/{id}/password-reset')
    def issue(id:str,data:Reason,req:Request):return p.issue_password_reset(id,actor=user(req,'operator',True),reason=data.reason)
    @app.get('/api/admin/audit')
    def audit(req:Request,offset:int=Query(0,ge=0),limit:int=Query(30,ge=1,le=100)):
        return p.access_audit(actor=user(req,'operator'),offset=offset,limit=limit)
