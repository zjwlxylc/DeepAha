...
class Contributions:
    ...
    def _submit_row(self,c,a,name,url,kind,note):
    ...
    def submit(self,a,name,url,kind,note,consent,key):
        ...
        with self.s.write() as c:
            return self.s.idempotent(c,a.name,'submit_site',key,dict(name=name,url=url,kind=kind,note=note,consent=consent),lambda:self._submit_row(c,a,name,url,kind,note))
