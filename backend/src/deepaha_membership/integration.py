"""Adapter integrated with the uploaded Mobile R2 Product; current verification in evidence/services-r3."""
...
class ProductPort:
    ...
    def task(self,a,id):
        a.need('operator');value=self.p.task_detail(id,actor=a.name)
        if value.get('revision_id'):
            from deepaha.product.models import Revision
            with self.p.db.tx(False) as session:
                rev=session.get(Revision,value['revision_id'])
                value['review_state']=rev.status if rev else None
        return value

    def owner_active(self,c,owner):
        from sqlalchemy import select
        from deepaha.product.models import Account
        return c.execute(select(Account.id).where(Account.username==owner,Account.active.is_(True))).scalar() is not None

    def notifications_enabled(self,c,owner):
        from sqlalchemy import select
        from deepaha.product.models import Account,Profile
        data=c.execute(select(Profile.data).join(Account,Account.id==Profile.account_id).where(Account.username==owner)).scalar() or {}
        return data.get('notification_enabled') is not False and data.get('notification_change_enabled') is not False
