...
class Commerce:
    ...
    def _create_order_row(self,c,a,code,version,note):
        p=self._plan(c,code)
    ...
    def create_order(self,a:Actor,code,version,key,note=''):
        ...
        with self.s.write() as c:
            return self.s.idempotent(c,a.name,'create_order',key,{'code':code,'version':version,'note':note},lambda:self._create_order_row(c,a,code,version,note))
