...
After schema writes, a failed switch keeps both services stopped; no old code is restarted.
"""
import hashlib,importlib.util,io,json,os,re,secrets,shutil,subprocess,sys,time,zipfile,tarfile
from pathlib import Path
BASE=Path('/opt/deepaha')
RUNTIME=BASE/'product-runtimes/mobile-r3-v1'
...
def root_for(build):
 if not re.fullmatch(r'mobile-r3-[a-zA-Z0-9-]{1,60}',build):raise RuntimeError('Invalid build')
...
def prepare(package,digest,build):
 ...
 needles=[v.encode() for c in configs for k,v in c.items() if k in ('DEEPAHA_WMA_API_KEY','DEEPAHA_WMA_AGENT_ID','DEEPAHA_WECHAT_APP_SECRET') and len(v)>=8]
 ...
 manifest=json.loads((dest/'MOBILE_R3_BUILD.json').read_text())
 ...
 if not RUNTIME.exists():
  ...
  run([BASE/'product-runtimes/experience-v1/bin/python','-m','venv',RUNTIME])
  old=next((BASE/'product-runtimes/experience-v1/lib').glob('python*/site-packages'))
  new=next((RUNTIME/'lib').glob('python*/site-packages'))
  shutil.copytree(old,new,dirs_exist_ok=True)
 run([RUNTIME/'bin/python','-m','pip','install','pypdf==6.19.0','Pillow==12.3.0'])
...
def pg_test(build):
 ...
 try:
  result=run(['runuser','-u','postgres','--',RUNTIME/'bin/python','-m','pytest','backend/tests/product/test_experience_postgres.py','backend/tests/product/test_services_postgres_r3.py','-q','--tb=short','-p','no:cacheprovider'],cwd=dest,env=env)
...
def deploy(build,env,expected_old):
 ...
 if env=='staging':
  stage_receipt=Path('/var/backups/deepaha/mobile-r3')/build/'staging-isolated/receipt.json'
 ...
 backup=Path('/var/backups/deepaha/mobile-r3')/build/env;backup.mkdir(parents=True,mode=0o700,exist_ok=False)
 ...
 receipt={'build':build,'environment':env,'old_release':str(old),'release':str(dest),'source_commit':json.loads((dest/'MOBILE_R3_BUILD.json').read_text())['source_commit'],'wma_validation_called':False,'backup':str(backup),'status':'IN_PROGRESS'}
 ...
 try:
  ...
  receipt['migration']=upgrade_services(p,backup/'pre-upgrade');migrated=True
  ...
  if dump.exists():
   ...
   try:
    ...
    for table,digest in before.items():
     ...
     with p.db.engine.connect() as conn:
      expected=conn.execute(text('SELECT count(*) FROM "'+table+'"')).scalar()
  ...
  receipt['r3_smoke']=r3_smoke(p,env)
  receipt['service_check']='PASS';success=True;receipt['status']='PASS'
 finally:
  if not success:
   migrated=migrated or any(name.startswith('mbr_') for name in inspect(p.db.engine).get_table_names())
   run(['systemctl','stop',worker]);run(['systemctl','stop',api])
   if pointer.resolve()==dest:switch(pointer,old)
   if active[api] and not migrated:run(['systemctl','start',api])
...
def r3_smoke(p,env):
 import httpx
 from deepaha.product.sharing import get_config,save_config
 sharing=get_config(p)
 if sharing['version']==0:
  actor='smoke_r3_'+secrets.token_hex(5);p.create_account(actor,secrets.token_urlsafe(24),['operator'])
  try:
   sharing['origin']='https://www.deepaha.com' if env=='staging' else 'https://staging.deepaha.com'
   save_config(p,(BASE/(env+'-current')).resolve()/'web/public/product',actor,sharing)
  finally:p.revoke_account(actor)
 from deepaha_membership.db import Store,plans,rows
 from sqlalchemy import select
 with Store(p.db.engine).read() as c:
  if any(x['status']!='DRAFT' for x in rows(c,select(plans))):raise RuntimeError('Plans unexpectedly published')
 origin='https://www.deepaha.com' if env=='staging' else 'http://127.0.0.1:8200'
 with httpx.Client(base_url=origin,trust_env=False,timeout=20) as c:
  paths=['/services','/share','/share-assets/default-wide.jpg','/share-assets/default-square.jpg','/api/site','/api/membership/plans','/api/membership/runtime-status']
  results={path:c.get(path) for path in paths}
  if any(r.status_code!=200 for r in results.values()):raise RuntimeError('R3 public routes failed')
  if results['/api/membership/plans'].json()!=[]:raise RuntimeError('Draft catalog is public')
  if results['/api/membership/runtime-status'].json()['enabled']:raise RuntimeError('R3 service unexpectedly enabled')
  build=results['/api/site'].json()['frontend_build']
  if not build.startswith('mobile-r3-'):raise RuntimeError('Wrong frontend build')
  if 'property="og:title"' not in results['/share'].text:raise RuntimeError('Sharing OG missing')
 return {'status':'PASS','frontend_build':build,'public_routes':len(paths),'plans':'DRAFT','periodic_service_enabled':False}
