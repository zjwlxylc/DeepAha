"""Root-only, isolated-staging-only release switch. Run from a reviewed local artifact.

Usage: python deploy_staging.py PACKAGE SHA256 EXPECTED_OLD_RELEASE BUILD_ID
No production/www, Nginx, Ops Gateway or WMA modifications.
"""
import hashlib
...
CURRENT=Path('/opt/deepaha/staging-isolated-current')
ENV=Path('/etc/deepaha/staging-isolated.env')
RUNTIME=Path('/opt/deepaha/product-runtimes/sg8a-v1/bin/python')
API='deepaha-api@staging-isolated'
WORKER='deepaha-worker@staging-isolated'
...
def main():
    ...
    old=CURRENT.resolve();live=Path('/opt/deepaha/staging-current').resolve()
    cfg=load_env(ENV,old);public=load_env('/etc/deepaha/staging.env',old)
    ...
    if cfg.get('DEEPAHA_WMA_ENABLED')!='0':raise RuntimeError('WMA must remain disabled on isolated staging')
    dest=Path('/opt/deepaha/staging-isolated-releases')/build
    if dest.exists():raise RuntimeError('Release already exists; do not overwrite')
    backup=Path('/var/backups/deepaha/staging-isolated')/build
    backup.mkdir(parents=True,mode=0o700,exist_ok=False);os.chmod(backup,0o700)
    receipt={'build':build,'old_release':str(old),'new_release':str(dest),'backup':str(backup),'www_modified':False,'wma_called':False}
    ...
    try:
        ...
        import httpx
        with httpx.Client(base_url='http://127.0.0.1:8200',headers={'Host':'staging.deepaha.com'},timeout=10) as client:
            for _ in range(30):
                try:
