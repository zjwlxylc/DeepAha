...
| `phase8-deadline-reminder` | Phase 8 分支和相关合并请求 | v0.7 契约、事件时点 audience、Outbox/重试/恢复、owner API、TEST_INBOX、Chromium 与 Phase 8 迁移往返 |
...
Phase 8 固定提醒夹具只报告 audience、candidate、attempt、TEST_INBOX 与测试通过数量。真实
参与者和真实供应商均为 `0`；这些数值不得命名或换算为打开率、送达率、投诉/关闭率、留存率或
行动指标。Phase 8 Release Qualification 保持 `NOT_STARTED`，v0.7 只能为 `IMPLEMENTED`，不得
标记 `STABLE`。Phase 6/7 真人 Release Qualification 与 Phase 8 Engineering 独立，不形成跨阶段
阻塞链，也不因 Phase 8 工程通过而得到真人或生产资格。
