...
本批实现冻结文本封装和读取校验，解决新持久化接入时 Python 与 JSONB 对数字表示不同的问题。PR #41 候选 `59640cd` 的 CI #137 全部 9 项成功，合并为 `abd1db9`，候选与合并树一致；证据 `evidence/2026-09-10-adjudication-frozen-storage-ci.json`。完整包沿用现有 digest，不从 model_dump 或数据库数值投影重新生成内容；旧时间戳拼写、原文、定位和上游快照保持不变。
