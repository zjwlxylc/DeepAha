...
状态 IMPLEMENTED，本地定向验证通过，候选 CI 待运行；不代表发布资格或实际人工验收通过。分支 `codex/group-rule-review`，基于 PR #30 同树合并后的 `8128fbf537984f792f2800fc6660da5aa93a7ef5`。
...
先核对本批精确候选的全部 CI，成功后按原授权合并并比较候选与合并代码树。随后接入桌面优先的组规则审核表单：沿用固定地址、当前事实核验、逐引用评估、显式提交、幂等键、保存后地址确认和过期后隐藏旧结果；只运行受影响的动作/组件/API 及浏览器保存与过期测试。保留完整字段分母，不增加自动批准或岗位继承。
