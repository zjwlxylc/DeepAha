...
内部服务 IMPLEMENTED；后续只读 API 与页面已经接入，见 `2026-09-10-group-applicability-ui.md`。尚无适用裁决写入。分支 `codex/group-rule-applicability`，基于 PR #32 已验证同树合并 `2fc37ee3d8fab08a93e9a730a933ea7f80cb68af`。本批不宣称整个 GROUP 适用链路完成。
...
2. 私有 GET API、严格响应契约、桌面管理页入口与过期/权限/失败隐藏行为已接入并定向验证；最新恢复点见 UI 交付说明。继续当前工作区和任务窗口；只有新增影响才重跑相关测试。
