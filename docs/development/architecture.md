...
早期允许 API、Worker、Scheduler 和 Evaluation Runner 共享同一 Python 包，但必须以独立进程入口运行。共享代码不等于共享职责。
...
| 模块 | 单一职责 | 不负责 |
| --- | --- | --- |
| `sources` | 官方源注册、抓取策略、健康和频率 | 解析业务字段 |
...
```text
Source poll
  -> RawArtifact (immutable bytes + hash)
...
- Redis 只用于缓存、队列、锁和限流；缓存丢失不能改变业务事实。
- pgvector 只用于去重候选、专业语义候选和检索，不用于硬资格最终裁决。
