...
if ([string]::IsNullOrWhiteSpace($projectName)) {
    $projectName = "deepaha-p9b-gold-$PID"
}
if ($projectName -cnotmatch '^deepaha-p9b-gold-[a-z0-9][a-z0-9-]*$') {
    throw "COMPOSE_PROJECT_NAME must use the deepaha-p9b-gold- prefix"
}
...
function Get-ExactProjectHostPorts {
    ...
    Assert-NativeSuccess "inspect exact Gold compose project"
    foreach ($containerId in $containerIds) {
        if ([string]::IsNullOrWhiteSpace($containerId)) {
            continue
        }
        $portJson = & docker inspect --format '{{json .NetworkSettings.Ports}}' $containerId
        Assert-NativeSuccess "inspect exact Gold container ports"
...
function Assert-PortAvailableOrOwned {
    ...
    if (-not $ownedPorts.Contains($Port)) {
        throw "Required Gold verifier port $Port is occupied outside compose project $projectName"
    }
}
...
try {
    ...
    Assert-NativeSuccess "start isolated Gold services"
    ...
    try {
        ...
        Assert-NativeSuccess "Gold migration upgrade"
        uv run pytest `
            tests/contracts/test_phase9b_contracts.py `
            tests/p9b/test_gold_governance.py `
            tests/p9b/test_gold_import.py `
            tests/p9b/test_benchmark.py `
            tests/p9b/test_benchmark_cli.py `
            tests/p9b/test_benchmark_contract_files.py `
            tests/test_p9b_gold_benchmark_verifier_scope.py `
            --strict-markers
        Assert-NativeSuccess "Gold contracts and deterministic benchmark tests"
        uv run pytest -m integration `
            tests/integration/test_p9b_gold_migration.py `
            --strict-markers
        Assert-NativeSuccess "Gold PostgreSQL and migration tests"
        uv run pytest -m integration --strict-markers
        Assert-NativeSuccess "Phase 1 through Phase 8 PostgreSQL regression"
        uv run alembic check
        Assert-NativeSuccess "Gold migration drift check"
        uv run alembic downgrade 20260824_0013
        Assert-NativeSuccess "Gold empty migration downgrade"
        uv run alembic upgrade head
        Assert-NativeSuccess "Gold migration re-upgrade"
        uv run alembic check
        Assert-NativeSuccess "Gold re-upgrade drift check"
    }
    finally {
        Pop-Location
    }
    ...
    try {
        & git diff --check
        Assert-NativeSuccess "Gold diff check"
    }
    finally {
        Pop-Location
    }

    Write-Host "P9-B GOLD GOVERNANCE AND BENCHMARK SLICE ENGINEERING GATE=CLOSED"
    Write-Host "Real Gold entries=0 / NOT_OBSERVED"
    Write-Host "Independent human evidence=0 / NOT_OBSERVED"
    Write-Host "Locked Acceptance benchmark=NOT_RUN"
    ...
    try {
        if ($cleanupProject) {
            ...
            Assert-NativeSuccess "remove exact Gold compose project"
        }
    }
    finally {
