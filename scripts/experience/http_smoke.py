    ...
    with tempfile.TemporaryDirectory(prefix='deepaha-http-smoke-') as data:
        ...
        with (out/'server.log').open('w',encoding='utf-8') as log:
            ...
            try:
                ...
                result={'scope':'Fresh isolated database and actual loopback HTTP; not TLS, native Windows or live WMA. No worker started.','checks':checks,'status':'PASS' if len(checks)>=25 and all(x['passed'] for x in checks) else 'FAIL','wma_called':False}
