...
class Harness:
 def __init__(self,pw,width=1440):
  ...
  def fetch(req):
   path=req['path'];r=self.client.request(req.get('method','GET'),path,headers=req.get('headers',{}),content=req.get('body'))
  ...
  self.page.evaluate('''assets=>{
   ...
   window.fetch=async(path,options={})=>{const r=await window.__qaFetch({path,method:options.method||'GET',headers:options.headers||{},body:options.body});return new Response(r.text,{status:r.status,headers:r.headers});};
