...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='deepaha-browser-') as data:
        ...
        with (out/'fixture-server.log').open('w',encoding='utf-8') as log:
            process=subprocess.Popen([sys.executable,str(ROOT/'scripts/experience/fixture_server.py')],cwd=ROOT,env=env,stdout=log,stderr=subprocess.STDOUT)
            try:
                ...
                with (out/'browser-check.log').open('w',encoding='utf-8') as report:
                    rc=subprocess.run([sys.executable,str(ROOT/'scripts/experience/browser_cases.py')],cwd=ROOT,env=env,stdout=report,stderr=subprocess.STDOUT,timeout=600).returncode
