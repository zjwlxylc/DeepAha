...
def main():
    ...
    ap.add_argument('--layout-only',action='store_true');ap.add_argument('--focused',action='store_true');ap.add_argument('--edges',action='store_true');ap.add_argument('--cases',choices=['browser_cases.py','share_cases.py'],default='browser_cases.py');args=ap.parse_args();out=args.output.resolve();out.mkdir(parents=True,exist_ok=True)
    ...
    with tempfile.TemporaryDirectory(prefix='deepaha-browser-') as data:
        ...
        with (out/'fixture-server.log').open('w',encoding='utf-8') as log:
            process=subprocess.Popen([sys.executable,str(ROOT/'scripts/services_r3/fixture_server.py')],cwd=ROOT,env=env,stdout=log,stderr=subprocess.STDOUT)
            try:
                ...
                with (out/'browser-check.log').open('w',encoding='utf-8') as report:
                    rc=subprocess.run([sys.executable,str(ROOT/'scripts/services_r3'/('focused_cases.py' if args.focused else 'edge_cases.py' if args.edges else args.cases))],cwd=ROOT,env=env,stdout=report,stderr=subprocess.STDOUT,timeout=600).returncode
