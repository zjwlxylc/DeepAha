#!/usr/bin/env python3
"""R3 current integrated product verification; no production DB or remote Agent calls."""
...
def main():
    ...
    run('asset-build',[sys.executable,'scripts/services_r3/build_assets.py','--check'])
    ...
    run('membership-regression',[sys.executable,'-m','pytest','validation/membership-s1/tests','-q','--junitxml='+str(out/'membership.xml')])
    run('compileall',[sys.executable,'-m','compileall','-q','backend/src','backend/tests/product','scripts/services_r3'])
    node=shutil.which('node') or 'node'
    for script in ['check-product','check-sg7-1','check-sg7-2','test-product-core','test-birth-date','test-experience','test-mobile-r2','test-mobile-r2-ops','test-sharing-r3']:
        run(script,[node,'scripts/'+script+'.mjs'],ROOT/'web')
    run('http-smoke',[sys.executable,'scripts/services_r3/http_smoke.py','--output',str(out/'http')])
    counts={}
    ...
    member={}
    if (out/'membership.xml').exists():
        root=ET.parse(out/'membership.xml').getroot();suites=list(root.iter('testsuite'));member={k:sum(int(x.get(k,'0')) for x in suites) for k in ('tests','failures','errors','skipped')};member['passed']=member['tests']-member['failures']-member['errors']-member['skipped']
    result={'status':'PASS' if all(x['returncode']==0 for x in stages) else 'FAIL','stages':stages,'counts':counts,'membership_counts':member,'wma_called':False,'postgres_live':'NOT_RUN','physical_mobile':'NOT_RUN','native_windows':'NOT_RUN','new_schema_tables':14}
