...
test("guided review completes two synthetic positions with plain questions and honest pending records", async ({ page, request }, testInfo) => {
  test.setTimeout(90000);
  await request.get("http://127.0.0.1:3097/seed-guided");
  await page.goto(`/review/investigations/${taskId}/workbench`);
  await expect(page.getByRole("heading", { name: "这次先核对哪两个岗位？" })).toBeVisible();
  await expect(page.getByRole("button", { name: "选满两个岗位后继续" })).toBeDisabled();
  await page.screenshot({ path: testInfo.outputPath("guided-select.png"), fullPage: true });
  await page.getByRole("checkbox", { name: /教学岗位/ }).focus();
  await page.keyboard.press("Space");
  await page.getByRole("checkbox", { name: /合成第二岗位/ }).check();
  await page.getByRole("link", { name: "继续：核对这两个岗位" }).click();
  await page.getByLabel("这是什么类型的机会？").selectOption("PUBLIC_INSTITUTION_JOB");
  await page.getByLabel("你核对了原件哪里？").fill("合成测试：已核对示例岗位表两行；不是实际真人签署");
  await page.getByRole("checkbox", { name: "我已对照原件，确认这些单位、岗位名称和编号对应正确" }).check();
  await page.getByRole("button", { name: "确认这两个岗位，开始逐条核对" }).click();
  await page.getByRole("button", { name: "准备核对内容" }).click();
  await expect(page.getByRole("group", { name: "对照原件，这条内容表达正确吗？" })).toBeVisible();
  await expect(page.getByRole("button", { name: "保存我的判断" })).toHaveCount(0);
  if (testInfo.project.name === "desktop") {
    const lastAnswer = await page.getByRole("radio", { name: "我现在无法判断" }).locator("..").boundingBox();
    expect(lastAnswer!.y + lastAnswer!.height).toBeLessThanOrEqual(page.viewportSize()!.height);
  }
  await page.screenshot({ path: testInfo.outputPath("guided-question.png"), fullPage: true });
  await page.getByRole("radio", { name: "是，原文支持这条内容" }).click();
  const key = await page.locator('input[name="request_key"]').inputValue();
  await page.getByRole("link", { name: "合成第二岗位 · P002", exact: true }).click();
  await page.getByRole("link", { name: "教学岗位 · P001", exact: true }).click();
  await expect(page.getByRole("group", { name: "这条要求适用于当前岗位吗？" })).toBeVisible();
  await expect(page.locator('input[name="request_key"]')).toHaveValue(key);
  await page.getByRole("radio", { name: "已核对岗位、共同条件及更正，没有未解决冲突" }).click();
  await page.getByRole("button", { name: "保存我的判断" }).click();
  await page.getByRole("link", { name: "继续下一条", exact: true }).click();
  await page.getByRole("button", { name: "归入岗位记录，继续" }).click();
  await page.getByRole("link", { name: "继续核对判断条件" }).click();
  await page.getByRole("button", { name: "准备判断条件" }).click();
  await page.getByRole("button", { name: "我现在无法判断" }).click();
  await page.getByRole("radio", { name: "生效时间只有日期，或没有说明" }).click();
  await page.getByRole("button", { name: "保存我的判断" }).click();
  await expect(page.getByText(/本人暂不能确认规则，原因：证据时间只有日期或不明确/)).toBeVisible();
  await page.getByRole("link", { name: "继续下一条", exact: true }).click();
  await page.getByRole("link", { name: "继续第二个岗位" }).click();
  await page.getByRole("radio", { name: "我现在无法判断" }).click();
  await page.getByRole("radio", { name: "看不懂，需要协助" }).click();
  const before = await (await request.get("http://127.0.0.1:3097/receipts")).json();
  await request.get("http://127.0.0.1:3097/drop-next-receipt");
  await page.getByRole("button", { name: "保存我的判断" }).click();
  await expect(page.getByRole("heading", { name: "这条判断已保存" })).toBeVisible();
  const after = await (await request.get("http://127.0.0.1:3097/receipts")).json();
  expect(after).toEqual({ mutations: before.mutations + 1, posts: before.posts + 1 });
  await page.getByRole("link", { name: "查看已保存与待处理" }).click();
  await expect(page.getByRole("heading", { name: "这次核对记录" })).toBeVisible();
  await expect(page.getByText("判断条件 1 条；仍需处理 1 条")).toBeVisible();
  await expect(page.getByText("原始内容 1 条 · 仍需处理 1 条")).toBeVisible();
  await page.reload();
  await expect(page.getByText("原始内容 1 条 · 仍需处理 1 条")).toBeVisible();
  if (testInfo.project.name === "desktop") await page.evaluate(() => { document.body.style.zoom = "2"; });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
