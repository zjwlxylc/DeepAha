...
export function field(f){return `<div class="fact" id="field-${e(f.id)}"><div class="fact-label">${e(f.label)} ${f.excluded?tag('不采用','red'):''}</div><div class="fact-value text-flow ${f.excluded?'excluded-value':''}">${e(f.value||'当前材料尚未提取此项')}</div>${f.value_total>f.value.length?`<button class="text-btn field-text-more" data-field="${e(f.id)}" data-offset="${f.value.length}">继续阅读</button>`:''}${(f.notes||[]).map(n=>`<div class="fact-note">${icon('info','sm')}<span>${e(n)}</span></div>`).join('')}${f.evidence?.length?`<details class="evidence"><summary>查看依据 ${f.quote_located?tag('引用已定位','green'):''}</summary>${f.evidence.map(ref=>`<div class="quote"><p class="text-flow">${e(ref.quote||'未提供引用片段')}</p><div class="between"><span class="muted small">${e(Object.entries(ref.locator||{}).map(([k,v])=>`${k} ${v}`).join(' · '))}</span>${outside(ref.url,'来源')}</div></div>`).join('')}</details>`:''}</div>`;}
export function fields(items){const sections=[...new Set(items.map(f=>f.section||'其他说明'))];return sections.map(s=>`<section class="field-section"><h3>${e(s)}</h3>${items.filter(f=>(f.section||'其他说明')===s).map(field).join('')}</section>`).join('');}
