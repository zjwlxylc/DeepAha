...
export function bind(selector,event,handler,scope=document){$$(selector,scope).forEach(el=>el.addEventListener(event,async ev=>{
 ...
 if(button?.disabled)return;activatedControl=button||el;if(button){button.disabled=true;button.classList.add('is-loading');button.setAttribute('aria-busy','true');}
...
export function modal(title,body,handler,submit='确定',danger=false){
 const trigger=activatedControl?.isConnected?activatedControl:document.activeElement,d=$('#dialog');activatedControl=null;if(d.open)d.close();d.onclose=()=>requestAnimationFrame(()=>{if(trigger?.isConnected&&!trigger.disabled)trigger.focus();});const heading='dialog-title-'+Date.now();d.setAttribute('aria-labelledby',heading);d.innerHTML=`<form class="dialog-form"><div class="between"><h2 id="${heading}">${e(title)}</h2><button type="button" class="icon-btn close-dialog" aria-label="关闭">${icon('x')}</button></div><div class="dialog-body">${body}</div><div class="dialog-footer"><button type="button" class="btn close-dialog">取消</button>${handler?`<button type="submit" class="btn ${danger?'danger':'primary'}">${e(submit)}</button>`:''}</div></form>`;
 $$('.close-dialog',d).forEach(b=>b.onclick=()=>d.close());
 if(handler)bind('form','submit',async(ev,form)=>{await handler(new FormData(form),form);if(d.contains(form))d.close();},d);
 // Explicitly wrap Tab at the dialog edges (native Chromium may otherwise
 // transfer focus to browser chrome). Leave Escape to the native dialog.
 d.onkeydown=event=>{
