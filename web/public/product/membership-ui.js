import {state,api as hostApi,modal,toast,go as hostGo,types} from './core.js';
const routePaths={services:'/services',account:'/app/subscription',feedback:'/app/sources',watches:'/app/tracking',notices:'/app/notifications',review:'/review/sources',manage:'/manage/services','manage-services':'/manage/services/items','manage-orders':'/manage/services/orders','manage-grants':'/manage/services/grants','manage-jobs':'/manage/services/jobs','manage-audit':'/manage/services/audit','manage-runtime':'/manage/services/runtime'};
export const membershipPaths=Object.values(routePaths).filter(p=>p!=='/app/notifications');
const api=(path,method='GET',data)=>hostApi('/api/membership/'+path,{method,data});
function go(route){hostGo(route==='login'?'/login?next='+encodeURIComponent(routePaths[currentRoute]):routePaths[route]||'/services');}
function render(){hostGo(location.pathname+location.search,true);}
...
function formDialog(title,html,submit,label='确认保存'){
 const key=uuid();
 modal(title,`<div class="membership-surface"><div class="form-grid">${html}</div></div>`,async d=>{
  await submit(d,key);toast('操作已保存');render();
 },label);
}
...
function orderDialog(p){
 ...
 const t=p.terms,custom=t.tier==='CUSTOM';
 let html=`<div class="field full"><div class="info-strip">${esc(t.name)} · ${t.months}个自然月 · ${money(t.price_cents)}<br>主题 ${t.topic_limit} 个；${t.site_limit?`网站 ${t.site_limit} 个；`:''}目标间隔 ${t.scan_hours} 小时。已有服务时从下一服务期生效。</div></div>`;
 html+=field('note',custom?'你希望持续关注什么？':'补充说明（可选）','','textarea',custom?'required minlength="5" maxlength="2000"':'maxlength="3000"',custom?'只描述需求，不填写密码、Cookie或个人敏感资料。':'这里只创建申请，不代表付款成功。');
 if(custom){
  html+=field('site_name','指定网站名称（可选）','','text','maxlength="200"');
  html+=field('site_url','公开网址（可选）','','url','maxlength="2048" placeholder="https://…"','填写网址后，提交申请时会同时送入网站线索审核，无需重复填写。');
  html+=`<div class="field full">${check('source_consent','提供网址时，同意平台核验后用于公共来源建设；身份和需求不公开。')}</div>`;
 }
 html+=`<div class="field full">${check('accepted','已核对范围、周期与价格，了解无自动续费及结果保证。')}</div>`;
 formDialog(custom?'申请定制服务':'确认服务申请',html,async(d,key)=>{
  if(!d.has('accepted'))throw new Error('请先确认服务范围与说明。');
  const data={code:p.code,version:p.version,key,note:d.get('note')||''};
  if(custom&&String(d.get('site_url')||'').trim()){
   if(!d.get('site_name'))throw new Error('请填写指定网站名称。');
   if(!d.has('source_consent'))throw new Error('请确认公开网站的提交授权。');
   await api('custom-requests','POST',{...data,site_name:d.get('site_name'),site_url:d.get('site_url'),consent:true});
  }else await api('orders','POST',data);
 },custom?'提交需求与网站':'创建服务申请');
}
...
function topicDialog(){formDialog('新增机会跟踪主题',field('name','给主题起个名字','','text','required maxlength="100"')+field('q','关键词','','text','maxlength="200" placeholder="例如：品牌、实习、校招"')+select('kind','机会类别',[['','全部类别'],...Object.entries(types)],'')+field('region','地区（可选）','','text','maxlength="100" placeholder="例如：宁波"'),async(d,key)=>api('watches/topics','POST',{name:d.get('name'),q:d.get('q'),kind:d.get('kind'),region:d.get('region'),key}),'保存主题');}
...
async function reviewView(){
 ...
 return section('网站线索审核','整体判断是否值得接入，不将线索审核等同于机会事实审核。',!info.demo?'<a class="btn secondary" href="/review/overview" data-nav>原机会审核 ↗</a>':'')+`<p class="muted small">按最新提交展示，最多200条。请勿在发给用户的回复或公开采集说明中包含个人隐私。</p><div class="stack section-label">`+(ls.length?ls.map(l=>{
...
const managerTabs=[['manage','套餐价格'],['manage-services','服务项目'],['manage-orders','申请与报价'],['manage-grants','用户权益'],['manage-jobs','跟踪任务'],['manage-audit','操作记录'],['manage-runtime','运行设置']];
function managerNav(route){return '<nav class="subnav" aria-label="服务管理分类">'+managerTabs.map(([r,t])=>`<a href="${routePaths[r]}" data-nav class="${r===route?'active':''}">${t}</a>`).join('')+'</nav>';}
async function manageView(route){
 let h=section('服务管理','套餐价格与服务项目由这里统一维护；新条款不追溯改变旧订单。')+managerNav(route);
 if(route==='manage-runtime')return await runtimeView();
 if(route==='manage'){
...
async function runtimeView(){
 const r=await api('manage/runtime');
 const edit=()=>{
  const fields=`<div class="field full">${check('enabled','启用周期服务检查',r.enabled)}${check('allow_site_enqueue','允许到期定制网站自动进入原采集队列，理解可能产生WMA费用',r.allow_site_enqueue)}</div>`+field('max_jobs','单轮网站任务上限',r.max_jobs,'number','required min="1" max="20"')+field('connection','已登记执行连接',r.connection,'text','required pattern="[a-zA-Z0-9_-]{1,48}"');
  formDialog('调整持续服务运行设置',fields,async d=>api('manage/runtime','PUT',{version:r.version,enabled:d.has('enabled'),allow_site_enqueue:d.has('allow_site_enqueue'),max_jobs:Number(d.get('max_jobs')),connection:d.get('connection')}));
 };
 const once=()=>formDialog('执行本轮服务检查',`<p class="field full">${r.allow_site_enqueue?'当前配置允许外部网站任务，可能产生采集费用。':'只检查已发布目录，不启动新的外部调查。'}此操作遵守账号、服务期、来源和预算限制。</p>`,async()=>{
  const result=await api('manage/runtime/check','POST',{});
  if(result.state==='FAILED')throw new Error('本轮检查未完成：'+result.code);
  toast(result.state==='COMPLETED'?'本轮检查已完成':'本轮状态：'+result.state);
 },'确认执行');
 return section('持续服务运行设置','复用原系统工作进程；未启用时不会自动安排网站调查。')+managerNav('manage-runtime')+`<div class="card"><div class="row between"><h2>${r.enabled?'周期检查已启用':'周期检查未启用'}</h2>${badge(r.allow_site_enqueue?'包含外部网站调查':'仅已发布目录')}</div><p class="details">最近一轮：${date(r.last_run)} · ${esc(r.last_result?.state||'尚未执行')}</p><p class="muted">工作进程每小时检查一次到期任务，按套餐目标间隔调度。单轮外部任务上限 ${r.max_jobs} 个。运行开关不等于实际执行成功。</p><div class="row section-label">${btn('调整运行设置',edit,'primary')}${btn('按当前设置执行一轮',once)}</div></div>`;
}
export async function renderMembership(path){
 currentRoute=Object.keys(routePaths).find(k=>routePaths[k]===path)||'services';
 const route=currentRoute;user=state.user;info={demo:false};actions.clear();
 if(route!=='services'&&!user)throw Object.assign(new Error('请先登录'),{status:401});
 let body;
 if(route.startsWith('manage')){
  if(!isOp())throw Object.assign(new Error('仅维护员可管理服务'),{status:403});
  body=await manageView(route);
 }else if(route==='review'){
  if(!isReviewer())throw Object.assign(new Error('仅审核员可处理网站线索'),{status:403});
  body=await reviewView();
 }else body=await ({services:servicesView,account:accountView,feedback:feedbackView,watches:watchesView}[route]||servicesView)();
 const nav=route==='services'||route==='account'||route==='feedback'||route==='watches'?'<nav class="subnav" aria-label="个人服务导航">'+[['services','服务方案'],['account','我的订阅'],['watches','持续跟踪'],['feedback','网站建议']].map(([k,v])=>`<a href="${routePaths[k]}" data-nav class="${route===k?'active':''}">${v}</a>`).join('')+'</nav>':'';
 if(['watches','account'].includes(route)){
  const status=await api('runtime-status');
  if(!status.enabled)body+='<div class="info-strip orange">自动服务检查尚未启用。网站提交、订单和跟踪记录已保存；实际检查以维护员启动及最近执行时间为准。</div>';
  else if(!status.last_run||Date.now()-new Date(status.last_run).getTime()>7200000)body+='<div class="info-strip orange">周期检查等待工作进程执行，请留意最近检查时间。</div>';
 }
 const handlers=new Map(actions);
 return {body:`<section class="membership-surface">${nav}${body}</section>`,after:()=>{
  const root=document.querySelector('#main-content .membership-surface');
  const media=matchMedia('(max-width:900px)'),workspace=path.startsWith('/manage')||path.startsWith('/review');
  const readonly=()=>{if(workspace)root?.querySelectorAll('[data-run]').forEach(b=>{b.disabled=media.matches;b.setAttribute('aria-disabled',String(media.matches));});};
  readonly();media.addEventListener('change',readonly);
  const click=async e=>{const b=e.target.closest('[data-run]');if(!b||b.disabled||!root.contains(b))return;const fn=handlers.get(b.dataset.run);if(!fn)return;
   if((path.startsWith('/manage')||path.startsWith('/review'))&&matchMedia('(max-width:900px)').matches){toast('手机工作台只读，请使用电脑操作');return;}
   b.disabled=true;try{await fn();}catch(err){toast(err.message);}finally{b.disabled=false;}
  };root?.addEventListener('click',click);return ()=>{root?.removeEventListener('click',click);handlers.clear();media.removeEventListener('change',readonly);};
 }};
}
