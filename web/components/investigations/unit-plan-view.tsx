...
export default function UnitPlanView({ snapshot, task }: { snapshot: InvestigationUnitSnapshot; task: InvestigationTask }) {
  ...
  return <>
    <header className="human-test-hero"><div><p className="eyebrow">内部核对 · 条件与依据</p><h1>{name} · 条件快照</h1><p>机会版本 {plan.target.opportunity_version} · 本目标版本 {plan.target.unit_version}</p><p>整理于 {formatDateTime(snapshot.created_at)}</p></div></header>
    ...
    <section className="human-test-panel" aria-labelledby="unit-announcement-applicability"><h2 id="unit-announcement-applicability">公告规则对本岗位的适用性</h2>
