...
def mount(app,product,settings=None,*,core=None,dns=None,demo=False):
    ...
    @app.get(prefix+'/info')
    def info():return {'version':'0.1.0','demo':demo,'payment':'MANUAL_ONLY','auto_renew':False,'baseline_integration_verified':False}
    ...
    @app.get('/membership/',include_in_schema=False)
    def shell():return FileResponse(static/'index.html',headers={'Cache-Control':'no-cache'})
