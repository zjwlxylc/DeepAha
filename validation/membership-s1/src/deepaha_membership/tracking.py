                ...
                with self.s.write() as c:
                    ...
                    for item in found:
                        ...
                        self.s.notice(c,w['owner'],key,str(item.get('title') or '主题机会有更新')[:240],
                            '跟踪主题：'+w['config']['name']+'。'+('来源变化待确认，请查看原机会中的备注。' if item.get('status')=='UPDATE_PENDING' else '此条目匹配你的筛选条件，不代表已判定符合报名资格。'),'watch:'+w['id'])
                        created+=int(not old)
                ...
                with self.s.write() as c:
                    ...
                    for item in found:
                        ...
                        self.s.notice(c,w['owner'],key,str(item.get('title') or '指定网站有新机会')[:240],
                            '指定网站：'+w['config']['name']+'。'+('来源变化待确认，请查看原机会备注。' if item['status']=='UPDATE_PENDING' else '该机会已进入机会总览；此提醒不是报名资格结论。'),'watch:'+w['id'])
                        created+=int(not old)
